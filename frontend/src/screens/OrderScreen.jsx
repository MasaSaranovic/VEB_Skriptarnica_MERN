...
import {
    useGetOrderDetailsQuery, usePayOrderMutation, useGetPaypalClientIdQuery
...
const OrderScreen = () => {
    ...
    async function onApproveTest() {
        await payOrder({
            orderId, details: { payer: { name: 'Test User' } }
        }).unwrap();
        refetch();
                ...
                <Col md={8}>
                    <ListGroup variant='flush'>
                        <ListGroup.Item>
                            ...
                            {order.isDelivered ? (
                                <Message variant='success'>Dostavljeno datuma:
                                    {order.deliveredAt}</Message>
                            ) : (
                                <Message variant='danger'>Nije
                                    dostavljeno</Message>
                            )}
                        </ListGroup.Item>
                        <ListGroup.Item>
                            ...
                            {order.isPaid ? (
                                <Message variant='success'>Plaćeno datuma:
                                ...
                                {order.orderItems.map((item, index) => (
                                    <ListGroup.Item key={index}>
                                        <Row>
                                            ...
                                            <Col md={4}>
                                                {item.qty} x
                                                {item.price.toFixed(2)} RSD = {(item.qty * item.price).toFixed(2)} RSD
                                            </Col>
                                        </Row>
                                    </ListGroup.Item>
                                ))}
                        ...
                        <ListGroup variant='flush'>
                            ...
                            <ListGroup.Item>
                                ...
                                <Row>
                                    ...
                                    <Col>{order.shippingPrice.toFixed(2)}
                                        RSD</Col>
                                </Row>
                                <Row>
                                    <Col>Pdv</Col>
